...
pub fn parse_sarif(text: &str) -> Result<SarifDocument> {
    ...
    let version = original
        ...
        .ok_or_else(|| TunerError::Validation("SARIF document missing version".to_string()))?;
    if version != "2.1.0" {
    ...
    let cwe_re = Regex::new(r"(?i)\bCWE[-_ ]?0*([0-9]{1,5})\b")
        .map_err(|err| TunerError::Validation(format!("internal SARIF CWE regex failed: {err}")))?;
    let mut findings = Vec::new();
    let runs = original
        .get("runs")
        .and_then(Value::as_array)
        .ok_or_else(|| TunerError::Validation("SARIF document missing runs".to_string()))?;
